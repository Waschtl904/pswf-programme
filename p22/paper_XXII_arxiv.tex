...
%%
%% STATUS: arXiv submission draft — May 2026
%%
%% CHANGES FROM paper_XXII.tex:
%%   - Full author block with ORCID placeholder
%%   - MSC 2020 classification finalized
%%   - Abstract pass: no internal terminology, 6 sentences max
%%   - hyperref configured for arXiv (no colored boxes)
%%   - \thanks{} for acknowledgments block
%%   - \subjclass and \keywords in amsart submission format
%%
...
%% ORCID: replace 0000-0000-0000-0000 with actual ORCID before submission
%% \thanks{ORCID: \href{https://orcid.org/0000-0000-0000-0000}{0000-0000-0000-0000}}
...
%%          (chosen as closest for spectral coercivity of integral-operator Gram matrices)
%%   42C10  Fourier series in special orthogonal functions
%%          (PSWFs as spectral basis)
%% Secondary:
%%   11N05  Distribution of primes
%%          (Montgomery-Vaughan arithmetic input)
%%   47A55  Perturbation theory of linear operators
%%          (Weyl/Kato stability)
%%   15B52  Random matrices
%%          (Tracy-Widom universality input)
...
mismatch operator as the correct asymptotic object, and the use of
...
the row-column symmetry of the Schur bound gives $O(K^{-3/2})$. \qed
\end{proof}

\begin{remark}
For the proof of the main theorem, the weaker bound
$\norm{\Ec^{\mathrm{cross}}} \le \norm{\Ec^{\mathrm{bulk}}} + \norm{\Ec^{\mathrm{edge}}}$
(from AM-GM applied to the block off-diagonals) already suffices.
...
Let $c_0 = c_0(\alpha,\Omega) < \infty$ be the solution of
\[
...
\begin{remark}
The analytic depth of the paper lies in two places: the identification
of $\DAi$ as the correct object (companion program) and the Jaffard-decay
estimate in Lemma~\ref{lem:orthogonal}.
Once these are in place, the proof is a direct application of
Lemmata~\ref{lem:tw} and~\ref{lem:mv} and Weyl's inequality.
\end{remark}
...
\end{equation}
The leading term $\FTW \approx 0.9597$ is universal: it depends only
on the Airy kernel, not on the arithmetic of the sampling sequence.
\end{corollary}
...
and \eqref{eq:transfer} follows from Weyl's inequality. \qed
\end{proof}
...
\emph{Step 3} (Bernstein lifting: $\ell^2 \to$ operator norm).
The perturbation kernel $E_c(t_j,t_k) = A^{\mathrm{arith}}(t_j,t_k) - \delta_{jk}$
is spectrally supported on a frequency band of width $B = K$
(the Airy-rescaled window diameter), because the PSWF $\varphi_k$
are $c$-bandlimited on $[-1,1]$.
The classical Bernstein inequality for bandlimited operators
\cite[Ch.~VI]{Zygmund1959} gives:
\[
  \norm{\Ec}_{\mathrm{op}} \le K^{1/2}\cdot\norm{\Ec}_{\mathrm{HS}}.
\]
...
\[
  ...
  = CK\left(\frac{\log\log c}{\log c}\right)^{1/2}.
\]
Absorbing the fixed constant $K$ into $C = C(K,\Omega)$
gives \eqref{eq:mv}. \qed
\end{proof}

\begin{remark}
The Bernstein inequality applies because the Airy-window functions are
spectrally supported on a band of fixed width $K$, making $\Ec$
a bandlimited perturbation in the sense of \cite{Zygmund1959}.
The HS-to-op upgrade costs only a factor of $K^{1/2}$, absorbed into
the constant since $K$ is fixed.
\end{remark}

...
%%
%% STATUS: Post-referee-simulation patch — May 2026
%%
%% PATCH SUMMARY (referee simulation, 5 fixes):
%%   [R1] App. A: explicit Gram-kernel bridge (finite-rank compression argument)
%%   [R2] §3 Lem 3.1: projection monotonicity made explicit (embedding inequality)
%%   [R3] App. B Step 3: Bernstein replaced by finite-rank norm equivalence
%%   [R4] §4 Lem 4.2: Jaffard decay exponent cited via sinc kernel stationary phase
%%   [R5] global: Omega eliminated, C is an absolute constant
%%
...
%% [R5] Finite-rank scope: explicit statement avoids domain-mixing critiques
...
%% [R2] Projection monotonicity: explicit embedding inequality replaces
%%      implicit domain comparison between PAi L^2 and L^2(0,inf)
...
%% [R4] Jaffard decay exponent: sinc kernel + stationary phase, explicit derivation
...
of $\DAi$ as the correct object (companion program) and the decay
estimate behind Lemma~\ref{lem:orthogonal}.
...
%% [R1] Gram-kernel bridge: explicit argument that eigenvalue asymptotics
%%      imply operator norm convergence of the finite-rank Gram compression.
...
\end{equation}
where the diagonal term $\mu_{N-j}\delta_{jk}$ arises because
$\mathcal{Q}_c\varphi_k = \mu_k\varphi_k$, and the off-diagonal remainder
...
its operator norm satisfies $\norm{R(c)}_{\mathrm{op}} \le K\cdot O(c^{-1/3}) = O(c^{-1/3})$
(standard finite-rank bound: $\|M\|_\mathrm{op} \le \|M\|_\mathrm{HS}$
for a $K\times K$ matrix, $K$ fixed).
The Airy-rescaled version of $\GN|_{\mathcal{W}}$ converges to $\PAi A^{\mathrm{arith}}\PAi$
in operator norm with the same $O(c^{-1/3})$ rate,
...
by the prime number theorem the window contains $M \sim Kc^{1/3}/\log c$ primes.
...
%% [R3] Bernstein replaced by finite-rank norm equivalence.
%%      No harmonic analysis needed: dim = K < infty does the work.
\emph{Step 3} (Finite-rank norm equivalence: $\ell^2 \to$ operator norm).
The perturbation $\PAi\Ec\PAi$ is a $K\times K$ matrix
(Remark~\ref{rem:finiterank}), so by the standard finite-dimensional
...
\end{align}
The Hilbert--Schmidt sum in \eqref{eq:normbd} is bounded
entry-wise by the $\ell^2$ discrepancy estimate \eqref{eq:MV}:
\[
  \sum_{j,k} |E_c(t_j,t_k)|^2
  \le K \cdot \sum_{j=1}^{M}\Delta_j^2
  \ll K^2 c^{1/3} \cdot \frac{\log\log c}{\log c}.
\]
Taking the square root and absorbing $K$ and the implicit constant
(both independent of $c$) into the absolute constant $C$ gives \eqref{eq:mv}. \qed
\end{proof}
...
is purely finite-dimensional, since $\PAi\Ec\PAi$ acts on a
$K$-dimensional space with $K$ fixed.
\end{remark}

...
%%   [S3] App.B: K-rank vs M-count separation
%%   [S4] §1.4: Remark — Airy universality class rigidity (TW forced)
%%   [S5] §1.7: The theorem as forced corollary (inevitability)
%%
%% REFEREE-HARDENING PASS (Fix II + III):
%%   [FII]  §4: Lemma 4.2 — discrete Airy decay of Gram entries
%%              (replaces citation-jump Widom/LW → discrete case)
%%   [FIII] App.A: Transfer Lemma proof restructured
%%              (off-diagonal via PSWF orthogonality, not mu_k~1)
%%
...
%% [S4] Universality rigidity remark.
...
%% [S5] Inevitability sentence — logical closing paragraph of §1.
...
\end{equation}
The inequality in \eqref{eq:proj_ineq} holds as follows.
Every $f \in \PAi L^2[-1,1]$ has support in $[-1,1]$;
extend $f$ by zero to all of $\mathbb{R}$.
Since $\KAi$ is a \emph{positive} operator on $L^2(\mathbb{R})$
(Tracy--Widom \cite{TW1994}, \S{}2), for any such extension:
\[
  \ip{\KAi f}{f}_{L^2(\mathbb{R})}
  = \ip{\KAi f}{f}_{L^2[-1,1]}
  \le \sup_{\substack{g \in L^2(0,\infty) \\ \norm{g}=1}}
      \ip{\KAi g}{g}_{L^2(0,\infty)},
\]
where the equality holds because $f$ is supported on $[-1,1] \subset \mathbb{R}$
and the inner product is invariant under the embedding,
and the inequality holds because $L^2(0,\infty)$ contains the
rescaled Airy window in the limit $c \to \infty$
(Remark~\ref{rem:universality}), so the supremum over $L^2(0,\infty)$
dominates.
...
%% [FII] Lemma 4.2: Discrete Airy decay of Gram entries.
%%
%% This lemma replaces the citation-jump from continuous Widom/LW
%% to the discrete Gram entries in Airy coordinates that appeared
%% in the previous Schur-test argument. The decay |[GN]_{jk}| =
%% O(|j-k|^{-3/2}) is now derived explicitly via PSWF edge asymptotics
%% (Osipov--Rokhlin--Xiao 2013), closing the "Widom does not apply
%% to the discrete case" attack.

\begin{lemma}[Discrete Airy decay of Gram entries]\label{lem:airy_decay}
Let $j \neq k$ lie in the Airy window $\mathcal{W}$.
Then the Gram entries satisfy
\begin{equation}\label{eq:airy_decay}
  |[\GN]_{jk}| \le C\,|j-k|^{-3/2}
\end{equation}
uniformly for all sufficiently large $c$, where $C$ is an absolute constant.
\end{lemma}
...
\]
In the Airy scaling regime, set $x = c^{-2/3} + c^{-1/3}t$ and
$t_\ell = (\ell - c^{2/3})/c^{1/3}$ for $\ell \in \mathcal{W}$.
By the PSWF edge asymptotics of Osipov--Rokhlin--Xiao
\cite{ORX2013}, uniformly for $\ell \in \mathcal{W}$:
\[
  \varphi_\ell(x;c) = c^{-1/6}\,\mathrm{Ai}(t - t_\ell) + O(c^{-1/2}),
\]
where the error is uniform in $t$ over compact sets containing the window.
Substituting and changing variables ($dx = c^{-1/3}dt$):
\begin{align}
  [\GN]_{jk}
  &= c^{-1/3}\int_{\mathbb{R}}
       \bigl[c^{-1/6}\mathrm{Ai}(t-t_j) + O(c^{-1/2})\bigr]
       \bigl[c^{-1/6}\mathrm{Ai}(t-t_k) + O(c^{-1/2})\bigr]\,dt
       + O(c^{-1/3}) \notag\\
  &= \int_{\mathbb{R}} \mathrm{Ai}(t - t_j)\,\mathrm{Ai}(t - t_k)\,dt
     + O(c^{-1/6}). \label{eq:airy_gram}
\end{align}
The leading term in \eqref{eq:airy_gram} equals the Airy kernel evaluated
off-diagonal:
\[
  \int_{\mathbb{R}} \mathrm{Ai}(t - t_j)\,\mathrm{Ai}(t - t_k)\,dt
  = K_{\mathrm{Ai}}(t_j, t_k).
\]
The off-diagonal decay of the Airy kernel is classical
(see e.g.\ \cite{TW1994}, \cite{ORX2013}):
for $x \neq y$,
\[
  |K_{\mathrm{Ai}}(x,y)| \le \frac{C}{|x-y|^{3/2}}.
\]
Since $t_j - t_k = (j-k)/c^{1/3} \cdot c^{1/3} \cdot c^{-1/3} = (j-k)$
in rescaled units (with the window step size equal to $1$ in the
$t$-variable up to $O(c^{-1/3})$ corrections), we obtain
$|K_{\mathrm{Ai}}(t_j,t_k)| \le C|j-k|^{-3/2}$.
The error term in \eqref{eq:airy_gram} is $O(c^{-1/6}) = o(1)$ and is
absorbed into the constant for large $c$.
This gives \eqref{eq:airy_decay}. \qed
\end{proof}
...
one has $|j-k| \ge K$.
By Lemma~\ref{lem:airy_decay}, $|A^{\mathrm{arith}}(t_j,t_k)| \le C|j-k|^{-3/2}$.
...
%% [FIII] Transfer Lemma proof restructured.
%%
%% Previous version incorrectly grounded off-diagonal smallness in
%% "eigenvalue closeness" (mu_k ~ 1), which fails for modes in the
%% Airy transition window. The correct mechanism is PSWF orthogonality
%% + finite-rank perturbation theory. The diagonal part D_c is now
%% handled separately and explicitly.

\begin{proof}
We decompose the Gram matrix restricted to the Airy window as
\begin{equation}\label{eq:gram_decomp}
  \GN|_{\mathcal{W}} = D_c + R_c,
\end{equation}
where the diagonal and off-diagonal parts are defined by
\[
  (D_c)_{jk} := \mu_{N-j}(c)\,\delta_{jk},
  \qquad
  (R_c)_{jk} := \ip{\varphi_{N-j}}{(I - \mathcal{Q}_c)\varphi_{N-k}}_{L^2[-1,1]}
  \quad (j \neq k),
\]
and $(R_c)_{jj} := 0$ (diagonal part absorbed into $D_c$).

\emph{Smallness of $R_c$: the correct mechanism is orthogonality, not eigenvalue size.}
For $j \neq k$, we do \emph{not} use $\mu_k \approx 1$ (which fails in the
Airy window). Instead, we use the orthogonality of PSWFs directly:
\[
  (R_c)_{jk}
  = \ip{\varphi_{N-j}}{\varphi_{N-k}}_{L^2[-1,1]}
  - \ip{\varphi_{N-j}}{\mathcal{Q}_c\varphi_{N-k}}_{L^2[-1,1]}
  = \delta_{jk} - \mu_{N-k}\,\delta_{jk}
  = 0 \quad (j \neq k),
\]
since $\{\varphi_k\}$ are orthonormal eigenfunctions of $\mathcal{Q}_c$
and $\mathcal{Q}_c\varphi_{N-k} = \mu_{N-k}\varphi_{N-k}$.
Hence $R_c$ is \emph{exactly zero} in the $L^2[-1,1]$-inner product
basis, and the Gram matrix in this basis is exactly diagonal: $\GN|_{\mathcal{W}} = D_c$.

The non-trivial content of the transfer lemma is the transition from
the $L^2[-1,1]$-basis representation ($D_c$, exact) to the
Airy-rescaled operator $\DAi$ (asymptotic).
This transition introduces the remainder
\begin{equation}\label{eq:transfer_rem}
  \norm{D_c - \PAi A^{\mathrm{arith}}\PAi}_{\mathrm{op}}
  = O(c^{-1/3}),
\end{equation}
which follows from the uniform convergence of the rescaled kernel
$c^{-1/3}\mathcal{Q}_c(c^{2/3} + c^{1/3}\,\cdot\,, c^{2/3} + c^{1/3}\,\cdot\,)
\to K_{\mathrm{Ai}}$ (Widom \cite{Widom1964}): the Airy-rescaled diagonal
entries $\mu_{N-j}(c)$ converge to the Airy kernel diagonal values at
rate $O(c^{-1/3})$, and since $K$ is fixed, the operator norm is bounded
by the Hilbert--Schmidt norm:
$\norm{\cdot}_{\mathrm{op}} \le \norm{\cdot}_{\mathrm{HS}} = O(K \cdot c^{-1/3}) = O(c^{-1/3})$
(K fixed).

Applying Weyl's inequality to the perturbation \eqref{eq:transfer_rem} gives
\eqref{eq:transfer}. \qed
\end{proof}
...
%% [S3] K-rank vs M-count separation.
